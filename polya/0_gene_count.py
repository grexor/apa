...
def save_figure(filename, x, y, legend):
    ...
    plt.xlim(x[0], x[-1])
    plt.xlabel("#Poly-A sites")
    plt.ylabel("#genes")
    plt.title("Poly-A sites per gene distribution")
    max_y = 0
    for vector in y:
        ax.plot(x, vector, linewidth=3, alpha=0.5, antialiased=True)
...
save_figure("signal.png", x, y, legend)
