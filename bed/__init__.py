...
def process_lib(lib_id, map_id=1, force=False):
    ...
    for exp_id, exp_data in lib.experiments.items():
        ...
        if exp_data["method"]=="pAseq":
            apa.bed.process_paseq(lib_id, exp_id=exp_id, map_id=1, force=force)
        if exp_data["method"]=="paseqx":
            apa.bed.process_paseqx(lib_id, exp_id=exp_id, map_id=1, force=force)

def process_exp(lib_id, exp_id, map_id=1, force=False):
    lib = apa.annotation.libs[lib_id]
...
def process_paseq(lib_id, exp_id, map_id, force=False):
...
def process_paseqx(lib_id, exp_id, map_id, force=False):
...
def process_e_lib(lib_id, force=False, map_id=1, polyid=None, genome=None):
    lib = apa.annotation.libs[lib_id]
    for exp_id, exp_data in lib.experiments.items():
        map_to = exp_data["map_to"]
        if genome!=None and genome!=map_to:
            continue
        if exp_data["method"]=="pAseq":
            apa.bed.process_e_paseq(lib_id, exp_id=exp_id, map_id=1, map_to=map_to, force=force)
        if exp_data["method"] in ["paseqx", "bed"]:
            apa.bed.process_e_paseqx(lib_id, exp_id=exp_id, map_id=1, map_to=map_to, polyid=polyid, force=force)

def process_e_exp(lib_id, exp_id, force=False, map_id=1, polyid=None):
    exp_id = int(exp_id)
...
def process_e_paseq(lib_id, exp_id, map_id, map_to, force=False):
    genome = apa.annotation.libs[lib_id].experiments[exp_id]["map_to"]
    r_filename = apa.path.r_filename(lib_id, exp_id)
...
def process_e_paseqx(lib_id, exp_id, map_id, map_to, polyid, force=False):
    genome = apa.annotation.libs[lib_id].experiments[exp_id]["map_to"]
    r_filename = apa.path.r_filename(lib_id, exp_id)
    if polyid==None:
        polyid = map_to
