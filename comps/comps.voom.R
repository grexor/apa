...
bitmap(file=paste(folder, "/", comps_id, ".expression.voom.mv.png", sep=""), width=1024, height=768, type="png16m", units="px", pointsize=16, taa = 4, gaa = 4)
y = voom(d, design, plot=T)
dev.off()

svg(paste(folder, "/", comps_id, ".expression.voom.mds.svg", sep=""), bg = "transparent")
plotMDS(y)
dev.off()
...
r.genes = topSplice(ex, coef=2, level="gene", n=Inf) # coef=2, compute change for test (1=control, 2=test); logFC >0: test > control
r.exons = topSplice(ex, coef=2, level="exon", n=Inf)

write.table(r.genes, file=paste(folder, "/", comps_id, ".expression.voom.genes.tab", sep=""), sep="\t", col.names=NA, quote=FALSE)
write.table(r.exons, file=paste(folder, "/", comps_id, ".expression.voom.exons.tab", sep=""), sep="\t", col.names=NA, quote=FALSE)
